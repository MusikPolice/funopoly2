...
import ca.jonathanfritz.monopoly.exception.MonopolyOwnershipException
...
class Bank(
    ...
    fun sellHouseToPlayer(
        ...
        val deed =
            player.deeds.keys.firstOrNull { it::class == propertyClass }
        ...
        if (!player.hasMonopoly(deed.colourGroup)) {
            throw MonopolyOwnershipException("${player.name} does not have monopoly on ${deed.colourGroup}") as Throwable
        }
    ...
    fun buyHouseFromPlayer(
        ...
        if (development.hasHotel) {
            throw PropertyDevelopmentException(
                "${player.name} must sell the hotel on ${propertyClass.simpleName} before selling any houses",
            )
        }
    ...
    fun sellHotelToPlayer(
        ...
        if (!deed.addingOrRemovingHotelRespectsEvenBuildingRules(player)) {
            throw PropertyDevelopmentException(
                "Properties in ${deed.colourGroup} are not sufficiently developed to allow building a hotel on ${propertyClass.simpleName}",
            )
        }
    ...
    fun buyHotelFromPlayer(
        ...
        if (!deed.addingOrRemovingHotelRespectsEvenBuildingRules(player)) {
            throw PropertyDevelopmentException(
                "Properties in ${deed.colourGroup} are not sufficiently developed to allow removing a hotel from ${propertyClass.simpleName}",
            )
        }
    ...
    private fun validatePlayerCanAffordDeed(
        player: Player,
        deed: TitleDeed,
    ): Int {
        if (player.money < deed.price) {
            throw InsufficientFundsException(
                $$"$${player.name} does not have enough money for this purchase ($$${player.money} < $$${deed.price})",
            )
        }
        return deed.price
    }

    private fun validatePlayerCanAffordDevelopment(
        player: Player,
        deed: TitleDeed,
    ): Int {
        ...
        if (player.money < buildingCost) {
            throw InsufficientFundsException(
                $$"$${player.name} does not have enough money for this purchase ($$${player.money} < $$$buildingCost)",
            )
        }
        return buildingCost
    }
